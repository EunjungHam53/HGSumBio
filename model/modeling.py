...
class LEDSeq2SeqModelOutput(ModelOutput):
    ...
    mgat_outputs: Optional[
        torch.Tensor] = None  # for each sample, only return the average embedding of all graph nodes
    sagpooling_outputs: Optional[
        torch.Tensor] = None  # for each sample, only return the average embedding of all graph nodes
...
class GAT(nn.Module):
    def __init__(self, input_channels, hidden_channels, out_channels):
        super().__init__()
        self.conv1 = GATConv(input_channels, hidden_channels, add_self_loops=False)
...
class LEDModel(LEDPreTrainedModel):
    _keys_to_ignore_on_load_missing = ["decoder.embed_tokens.weight", "encoder.embed_tokens.weight", "gat.weight",
                                       "sagpooling.weight"]
    ...
    def __init__(self, config: LEDConfig):
        ...
        self.gat = GAT(input_channels=config.d_model, hidden_channels=config.d_model, out_channels=config.d_model)
        self.sag_pooling = SAGPooling(in_channels=config.d_model, ratio=0.6)
    ...
    def forward(
        ...
        for index, last_hidden_state_sample in enumerate(encoder_outputs.last_hidden_state):
            ...
            hgat_output = hgat(heterograph_sample.x_dict, 
                   {k: v.long() for k, v in heterograph_sample.edge_index_dict.items()})

            if docs_positions_source != None:
                hgat_average_embedding = (torch.mean(hgat_output["token"], 0, False) + torch.mean(hgat_output["sent"],
                                                                                                  0,
                                                                                                  False) + torch.mean(
                    hgat_output["doc"], 0, False)) * 1 / 3
            else:
                hgat_average_embedding = (torch.mean(hgat_output["token"], 0, False) + torch.mean(hgat_output["sent"],

                                                                                                  0, False)) / 2
            # print(hgat_average_embedding.size())
            hgat_average_embeddings.append(hgat_average_embedding.tolist())

            if docs_positions_source != None:
                heterograph_sample["sent"].x = hgat_output["sent"]
                sagpooling_output = self.sag_pooling(heterograph_sample["sent"].x,
                                                     heterograph_sample[
                                                         'sent', 'similar_sent_sbert', 'sent'].edge_index)
                embeddings_sents_compressed = sagpooling_output[0]
                nodes_sents_compressed = sagpooling_output[-2]
                positions_sents_compressed = torch.index_select(sents_positions_source_sample, 0,
                                                                nodes_sents_compressed)

                nodes_tokens_compressed = []
                sent_token_ls = heterograph_sample['sent', 'sent_contain_token', 'token'].edge_index[0]
                sent_token_rs = heterograph_sample['sent', 'sent_contain_token', 'token'].edge_index[1]
                for l, r in zip(sent_token_ls, sent_token_rs):
                    if l in nodes_sents_compressed:
                        nodes_tokens_compressed.append(r)
                positions_tokens_compressed = torch.index_select(words_positions_source_sample, 0,
                                                                 torch.LongTensor(nodes_tokens_compressed).to(device))
                embeddings_tokens_compressed = torch.index_select(last_hidden_state_sample, 0,
                                                                  positions_tokens_compressed)

                nodes_docs_compressed = []
                doc_sent_ls = heterograph_sample['doc', 'doc_contain_sent', 'sent'].edge_index[0]
                doc_sent_rs = heterograph_sample['doc', 'doc_contain_sent', 'sent'].edge_index[1]
                for l, r in zip(doc_sent_ls, doc_sent_rs):
                    if r in nodes_sents_compressed:
                        nodes_docs_compressed.append(l)
                positions_docs_compressed = torch.index_select(docs_positions_source_sample, 0,
                                                               torch.LongTensor(nodes_docs_compressed).to(device))
                embeddings_docs_compressed = torch.index_select(last_hidden_state_sample, 0, positions_docs_compressed)

                sagpooling_average_embedding = (torch.mean(embeddings_sents_compressed, 0, False) + torch.mean(embeddings_tokens_compressed, 0,
                                                                                    False) + torch.mean(
                        embeddings_docs_compressed, 0, False)) * 1 / 3
                sagpooling_average_embeddings.append(sagpooling_average_embedding.tolist())

                # update decoder inputs based on compressed graph
                for pos, token_emb in zip(positions_tokens_compressed, embeddings_tokens_compressed):
                    last_hidden_state_sample[pos] = token_emb
                for pos, sent_emb in zip(positions_sents_compressed, embeddings_sents_compressed):
                    last_hidden_state_sample[pos] = sent_emb
                for pos, doc_emb in zip(positions_docs_compressed, embeddings_docs_compressed):
                    last_hidden_state_sample[pos] = doc_emb
                decoder_input_new.append(last_hidden_state_sample.tolist())

                # update cross attention mask
                attention_mask_sample = attention_mask[index % len(attention_mask)]
                attention_mask_sample_new = torch.zeros_like(attention_mask_sample)
                attention_mask_sample_new.type_as(attention_mask_sample)
                attention_mask_sample_new[0] = 1
                attention_mask_sample_new[-1] = 1
                for pos in positions_tokens_compressed:
                    attention_mask_sample_new[pos] = 1
                for pos in positions_sents_compressed:
                    attention_mask_sample_new[pos] = 1
                for pos in positions_docs_compressed:
                    attention_mask_sample_new[pos] = 1
                encoder_attention_mask_new.append(attention_mask_sample_new.tolist())

        hgat_average_embeddings = torch.tensor(hgat_average_embeddings).to(device)
        if docs_positions_source != None:
            # decoder outputs consists of (dec_features, past_key_value, dec_hidden, dec_attn)
            # pdb.set_trace()
            sagpooling_average_embeddings = torch.tensor(sagpooling_average_embeddings).to(device)
            decoder_input_new = torch.tensor(decoder_input_new).to(device)
            encoder_attention_mask_new = torch.tensor(encoder_attention_mask_new).to(device)
            decoder_outputs = self.decoder(
                input_ids=decoder_input_ids,
                attention_mask=decoder_attention_mask,
                encoder_hidden_states=decoder_input_new,
                encoder_attention_mask=encoder_attention_mask_new,
            ...
            return LEDSeq2SeqModelOutput(
                ...
                mgat_outputs=hgat_average_embeddings,
                sagpooling_outputs=sagpooling_average_embeddings
            )
        else:
            return LEDSeq2SeqModelOutput(
...
class LEDForConditionalGeneration(LEDPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            print("LEDForConditionalGeneration", return_dict)
        ...
        return LEDSeq2SeqLMOutput(
            ...
            mgat_outputs=outputs.mgat_outputs,
